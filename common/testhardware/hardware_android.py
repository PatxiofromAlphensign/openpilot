else: return None def set_battery_charging(self, on): return self.read_param_file("/sys/class/power_supply/battery/capacity", int, 100) f.write(f"{1 if on else 0}\n") return self.read_param_file("/sys/class/power_supply/usb/present", lambda x: bool(int(x)), False) TODO: build using methods from this class return self.read_param_file("/sys/class/power_supply/battery/charge_type", lambda x: x.strip() != "N/A", True) This does not correspond with actual charging or not.  NetworkType = log.ThermalData.NetworkType
 We don't have a good direct way to measure this on android   def service_call(call):
 If a USB cable is plugged in, it responds with 'Charging', even when charging is disabled     ret = subprocess.check_output(["service", "call", *call], encoding='utf8').strip()
